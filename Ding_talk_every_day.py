...
class DingTalkBotEnhanced:
	# 钉钉允许的图片域名白名单
	WHITELIST_DOMAINS = ["alicdn.com", "qiniucdn.com", "aliyuncs.com", "cdn.", "img.", "oss-", "yourcompany.com"]

	# 钉钉屏蔽的域名列表
	BLOCKED_DOMAINS = ["github.com", "raw.githubusercontent.com", "localhost", "127.0.0.1"]
	...
	def __init__(self, webhook: str, secret: Optional[str] = None, custom_img_file_name: str = None,
	             custom_config_file_name: str = None):
		"""初始化钉钉机器人
	...
	def validate_image_url(url: str) -> tuple[bool, str]:
		...
		# 检查文件格式
		if not re.search(r"\.(" + "|".join(DingTalkBotEnhanced.SUPPORTED_IMAGE_FORMATS) + r")$", url, re.IGNORECASE):
	...
	def send_markdown_with_image(self, title: str, content: str, img_url: str, image_placeholders: List[str] = None,
		...
		# 3. 发送Markdown（带重试机制）
		return self._send_with_retry(method=self.bot.send_markdown, title=title, text=content,
			at_mobiles=at_mobiles if not is_at_all else None, is_at_all=is_at_all)
	...
	def send_text(self, msg: str, at_mobiles: Optional[List[str]] = None, is_at_all: bool = False) -> bool:
		"""发送文本消息
		...
		Returns:
			bool: 发送是否成功
		"""
		final_msg = ("@所有人 " if is_at_all else " ".join([f"@{m}" for m in at_mobiles]) + " ") + msg if (
					is_at_all or at_mobiles) else msg

		return self._send_with_retry(method=self.bot.send_text, msg=final_msg, at_mobiles=at_mobiles,
			is_at_all=is_at_all)
		...
		if "start_date" in schedule and "end_date" in schedule:
			try:
				...
				if start_datetime <= current_datetime <= end_datetime:
					logger.info("📅 匹配到日期范围: %s 至 %s | 配置: %s", schedule["start_date"], schedule["end_date"],
					            schedule["name"])
					current_config_file_name = schedule["config_file"]
					current_img_file_name = schedule["image_file"]
					return current_config_file_name, current_img_file_name
				...
				logger.warning("⚠️ 日期格式错误: %s 至 %s，已跳过", schedule["start_date"], schedule["end_date"])
				continue
			...
			for idx, item in enumerate(config, 1):
				...
				try:
					...
					if item['send_type'] == "markdown_with_image":
						# Markdown带图片：替换所有占位符
						if bot.send_markdown_with_image(title=item['title'], content=content, img_url=img_url,
								image_placeholders=item['placeholders'], at_mobiles=item['at_mobiles'],
								is_at_all=item['is_at_all']):
							success_count += 1
					elif item['send_type'] == "markdown":
						# 纯Markdown：不处理图片
						if bot._send_with_retry(method=bot.bot.send_markdown, title=item['title'], text=content,
								at_mobiles=item['at_mobiles'], is_at_all=item['is_at_all']):
							success_count += 1
					elif item['send_type'] == "text":
						# 纯文本：不处理图片
						if bot.send_text(msg=content, at_mobiles=item['at_mobiles'], is_at_all=item['is_at_all']):
							success_count += 1
					else:
						logger.error("❌ 不支持的发送类型: %s", item['send_type'])
						success_count += 1
				except Exception as e:
					logger.error("❌ 处理第 %d 项时出错: %s | Webhook: %s", idx, str(e), item['webhook'][-10:])
					continue
